...
type HealthController struct {
	...
	todoService core.TodoListService // 待办事项服务
}

// NewHealthController 创建HealthController实例
func NewHealthController(todoService core.TodoListService) *HealthController {
...
func (controller *HealthController) Health(c *gin.Context) {
	...
	// 检查待办事项服务
	todoHealth := controller.checkTodoService()
	response.Components["todo_service"] = todoHealth
...
// checkTodoService 检查待办事项服务健康状态
func (controller *HealthController) checkTodoService() *ComponentHealth {
	start := time.Now()
	...
	if controller.todoService != nil {
		count, err := controller.todoService.Count(ctx)
		if err != nil {
			logger.Error("获取待办事项总数失败", zap.Error(err))
			return &ComponentHealth{
				Status:    HealthStatusDegraded,
				Message:   "待办事项服务状态获取失败: " + err.Error(),
				Timestamp: time.Now(),
				Duration:  time.Since(start).String(),
			}
		} else {
