...
// TaskStatsHourly 任务每小时统计数据（可选，用于更精细的实时监控）
//
// 存储每小时的任务执行统计，用于实时监控和告警
// 可选功能，如果需要更精细的监控可以启用
//
// 设计要点：
// - StatHour: 统计小时（精确到小时）
// - 数据保留策略：只保留最近7天的数据，超过7天的数据删除
// - 用于实时告警和WebSocket推送
type TaskStatsHourly struct {
	ID     uuid.UUID  `gorm:"size:36;primaryKey" json:"id"`               // 统计记录唯一标识
	TeamID *uuid.UUID `gorm:"size:36;index:idx_team_hour" json:"team_id"` // 团队ID，用于多租户隔离

	// 统计小时
	StatHour time.Time `gorm:"type:datetime;index:idx_team_hour;index:idx_stat_hour" json:"stat_hour"` // 统计小时（精确到小时）

	// 🔥 任务数量统计
	TotalTasks   int `gorm:"type:int;default:0" json:"total_tasks"`   // 总任务数
	SuccessTasks int `gorm:"type:int;default:0" json:"success_tasks"` // 成功任务数
	FailedTasks  int `gorm:"type:int;default:0" json:"failed_tasks"`  // 失败任务数

	// 🔥 队列健康度（每小时采样）
	PendingCount int `gorm:"type:int;default:0" json:"pending_count"` // 该小时的pending任务数
	RunningCount int `gorm:"type:int;default:0" json:"running_count"` // 该小时的running任务数
...
// TableName 表名
func (TaskStatsHourly) TableName() string {
	return "task_stats_hourly"
}

// BeforeCreate GORM钩子：创建前生成UUID
func (m *TaskStatsHourly) BeforeCreate(tx *gorm.DB) error {
	if m.ID == uuid.Nil {
		m.ID = uuid.New()
	}
	return nil
