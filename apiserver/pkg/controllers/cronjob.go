...
// Create 创建定时任务
// @Summary 创建定时任务
// @Description 创建新的定时任务，支持cron表达式调度。如果提供X-TEAM-ID，任务将归属于该团队
// @Tags cronjobs
// @Accept json
// @Produce json
...
// List 获取定时任务列表
// @Summary 获取定时任务列表
// @Description 获取定时任务列表，支持分页和过滤。如果提供X-TEAM-ID，则只返回该团队的任务。通过view_all_teams参数可以查看跨团队数据：管理员查看所有团队，普通用户查看自己所属的所有团队
// @Tags cronjobs
// @Accept json
// @Produce json
