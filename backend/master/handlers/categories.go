...
func CategoryCreate(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	// log.Println(category)
	// 保存category到etcd中
	if _, err = etcdManager.SaveCategory(category, true); err != nil {
		goto ERR
	} else {
		// 返回结果
...
// 分类更新操作
// Method: PUT
func CategoryUpdate(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	// 定义变量
	var (
		contentType   string
		needParseForm bool
	...
	// log.Println(category)
	// 保存category到etcd中: 更新
	if _, err = etcdManager.SaveCategory(category, false); err != nil {
		goto ERR
	} else {
		// 返回结果
		if categoryData, err = json.Marshal(category); err != nil {
