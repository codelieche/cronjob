...
func RsyncDingDingData(ding *common.DingDing) (err error) {
	var (
		dingDepartments []*common.DingDepartment
	...
	if dingDepartments, err = ding.ListDepartment(); err != nil {
		...
		for _, dingDepartment := range dingDepartments {
			...
			log.Println(msg)
			if err = rsyncDepartmentUser(ding, department); err != nil {
...
// 同步部门用户
func rsyncDepartmentUser(ding *common.DingDing, department *datamodels.Department) (err error) {
	var (
		offSet   int
		pageSize int
